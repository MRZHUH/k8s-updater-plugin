...
- List Resources
  ...
  - Params: `resourceType` (supports short names: `no/pod/deploy/sts/ds/svc/ing`, default `pod`), `namespace` (optional)
...
- Get Resource
  ...
  - Params: `resourceType`, `name` (optional), `namespace` (default `default`), `outputFormat` (`json` or `yaml`, default `json`)
  - Behavior: when `name` is empty, list resources; for cluster-scoped kinds (e.g. `node`) it lists all
